...
module.exports = {
  ...
  isPathWhitelisted: function(whitelist, absoluteUrlString, includeParams) {
    ...
    var path;
    if (includeParams) {
      var url = new URL(absoluteUrlString);
      path = url.pathname + url.search;
    } else {
      path = (new URL(absoluteUrlString)).pathname;
    }
  ...
  isPathBlacklisted: function(blacklist, absoluteUrlString, includeParams) {
    // If the blacklist is empty, then consider no URLs to be blacklisted.
    ...
    return blacklist.some(function(blacklistedPathRegex) {
      return path.match(blacklistedPathRegex);
    });
  },
